...
namespace tterm.Terminal
{
    internal class TerminalBuffer
    {
        ...
        public void ShiftUp()
        {
            ...
            //if (_buffer != null)//wayne
            //{
            //    Debug.WriteLine(_buffer[15 * _size.Columns].ToString() + _buffer[15 * _size.Columns + 1].ToString());
            //}
            //上移一行时，最后一行的文字没有清空
            for (int y = 0; y < _size.Rows -1; y++)
            {
                ...
                if (y == _size.Rows - 2)
                {
                    ClearBlock(0, y + 1, _size.Columns, y + 1);
                }
            }
        ...
        public TerminalTagArray GetFormattedLine(int y)
        {
            ...
            (var buffer, int startIndex, int endIndex) = GetLineBufferRange(y);
            //if (buffer != null)//wayne
            //{
            //    Debug.WriteLine(buffer[y * _size.Columns].ToString() + buffer[y * _size.Columns + 1].ToString());
            //}
            if (buffer == null)
            {
